...
T SingleLinkedList<T>::getCurrentVal() const
{
    if (currentPos == nullptr) {
        throw std::out_of_range("Empty current position! Can't get value!");
    }
...
{
    if (currentPos == nullptr || head == nullptr || head->next == nullptr) {
        return;
    }
    ...
    if (toDelete != nullptr) {
        ...
        if (currentPos->next == nullptr) {
            currentPos = nullptr; // If we removed the last element
        }
    }
}
...
bool SingleLinkedList<T>::find(const T &_val)
{
    ...
    currentPos = nullptr; // Reset currentPos if not found
    return false;
}
